  ...
  create(context) {
    return {
      ...
      'CallExpression:exit'(node) {
        ...
        const callerMethodNameIndex = methodStack.indexOf(calledMethodName);
        if (callerMethodNameIndex === -1 || callerMethodNameIndex >= methodStack.length - 1) {
          return;
        }

        context.report({
          node,
          message: `'${calledMethodName}' is called before its aa definition in the class.`,
        });
      }
    };
  },
